...
namespace SAutoCarry.Champions.Helpers
{
    public static class Tumble
    {
        ...
        public static void MoveAndWallTumble(Vector2 pos, Vector2 pos2)
        {
            ...
            if (ObjectManager.Player.ServerPosition.To2D().Distance(pos) > 1500 || !s_Champion.Spells[Champion.Q].IsReady())
                return;
            ...
            if(ObjectManager.Player.ServerPosition.To2D().Distance(pos) <= 10f)
            {
                if (s_NextWallTumbleWalkPos != pos)
                {
                    ...
                    ObjectManager.Player.IssueOrder(GameObjectOrder.MoveTo, pos.To3D());
                }
            ...
            if(!ObjectManager.Player.IsMoving)
            {
                ...
                ObjectManager.Player.IssueOrder(GameObjectOrder.MoveTo, s_NextWallTumbleWalkPos.To3D());
            }
            else
            {
