...
namespace SAutoCarry.Champions
{
    public class Cassiopeia : Champion
    {
            ...
            if (Spells[R].IsReady())
            {
                ...
                else
                {
                    ...
                    if (pred.HitCount >= ComboUseRMin)
                    {
                        ...
                        {
                            if (col.Type == GameObjectType.obj_AI_Hero)
                            {
        ...
        public void KillSteal()
        {
            ...
            foreach (Obj_AI_Hero target in HeroManager.Enemies.Where(x => x.IsValidTarget(Spells[E].Range) && !x.HasBuffOfType(BuffType.Invulnerability) && (!KillStealOnlyPoison || x.HasBuffOfType(BuffType.Poison))))
            {
